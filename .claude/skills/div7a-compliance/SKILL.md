...
7. **Output the register.** Per counterparty: opening balance, movements, agreement status, minimum repayment vs actual, exposure, action required before lodgment day. Use the firm-approved secure client-data location. If none is configured, ask before creating a path beside a checkout. Confirm the selected path is outside every version-control checkout, not merely ignored by one; do not change `.gitignore`, output locations or repository configuration without explicit approval.
