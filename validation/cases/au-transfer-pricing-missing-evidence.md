---
id: au-transfer-pricing-missing-evidence
synthetic: true
target_skills:
  - au-transfer-pricing
---

# Transfer pricing evidence pack with missing evidence

## Scenario

Synthetic Entity A supplies fabricated records for the task below. All records are invented for this case.

A related-party service fee has invoices but no benefit evidence or benchmarking. Record the documentation gap rather than declaring the markup arm's length.

## Task

Use `au-transfer-pricing` to prepare the supported work and an unresolved-items hand-off.
...
- A transaction reconciliation, functional evidence matrix and documentation gaps. The specialist decides method, pricing and disclosures; the agent gives no compliance certification.
